...
広告収益は目的ではない。
**PVは Windsor.ai 経由のGA4から直接読める**（アカウント `271991344`）。CSVの書き出しは要らない。
取り方と、2026-09-09に見た結果は `docs/PV-2026-09.md`。

**サーチコンソールは gouter.works がつながっていない**（Windsor.ai には `tsukinuke.jp` だけ）。
順位とクエリが要るときは管理画面からCSVを書き出して読む。
