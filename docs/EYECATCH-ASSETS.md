...
| **インスタ用** | Goûterインスタ カルーセル 38 値上げの伝え方 | `DAHUt3plSVk` |
...
### テキストを差し替えるとフォントが変わる条件（2026-09-09に特定）

「まっさらなページに足すと変わる」だけではなかった。**既存のテキストを差し替えても変わる**場合がある。
インスタのカルーセルを作るときに全部踏んだので、条件を書いておく。

母艦のテキスト要素は2種類ある。**見た目ではなく、字間と行送りで見分ける。**

| 種類 | 字間・行送り | `replace_text` したとき |
|---|---|---|
| 小さい方（上段・下段） | `letterSpacing 0` / `lineHeight 1.4` | **`YADK30shh-I` のまま** |
| 大きい表示用（中央） | `letterSpacing -0.077` / `lineHeight 1.1` | **`YACgEZ1cb1Q` に変わる** |

小さい方でも、次を入れると変わる。

- **改行（`\n`）を入れる**
- **日本語フォントに無い記号を入れる**（`→` で実際に起きた）

安全なのは次のとおり。

- **小さい要素だけを使い、`format_text` の `font_size` で大きくする。** サイズ変更はフォントに影響しない
- **行を分けたいときは改行を使わない。** 幅を狭めて**自動で折り返させる**（自動折り返しは安全）。
  要素を2つ使ってもよい
- **英数字は安全。** 母艦の `1WAY-3JOB` も、カルーセルの `DM` も `YADK30shh-I` のまま出る

**壊れたら `replace_text` では戻せない。** 日本語だけの文字列に戻しても `YACgEZ1cb1Q` のまま。
**トランザクションを `cancel` してやり直すのが唯一の復旧方法。** だから
**コミットする前に、必ず `document` の `fontRef` を目で確かめる。** サムネイルだけでは気づけない。
